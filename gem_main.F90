...
subroutine grid1(ip,n)
    ...
    do m=1,mme
        ...
        if(psp>=3.340450e-1 .and. psp<=4.630094e-1)then
           wght=wght*(4.630094e-1-psp)/(4.630094e-1-3.340450e-1)
        endif
#endif
    ...
    999 continue
#ifdef __GEM_XGC_COUPLING
    call cce_send_density_electron(dene)
    call cce_send_current_electron(upar)
#endif
...
subroutine accumulate(n,ip)
    ...
    if(myid==0)then
      ...
      if(init==0)then
        call adios2_declare_io(io,adios2obj,'output_3d',err)
        call adios2_define_variable(var, io, "gem_data", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "xgc_data", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        init=1
      endif
      ...
      call adios2_put(engine, "gem_data", density_3d, err)
      call adios2_put(engine, "xgc_data", pot_density_3d, err)
...
#else
    call cce_receive_as(apars)
    call enfxy(apars(:,:,:))
    call enfz(apars(:,:,:))
    !call cce_send_as(apars)
#endif
...
subroutine reporter(n)
    ...
    integer :: err
    real :: e=1.6022e-19
    real :: iden_xgc(cce_all_node_number,nphi),eden_xgc(cce_all_node_number,nphi),ijpar_xgc(cce_all_node_number,nphi),ejpar_xgc(cce_all_node_number,nphi),phi_xgc(cce_all_node_number,nphi),Ah_xgc(cce_all_node_number,nphi),As_xgc(cce_all_node_number,nphi)
    real :: iden_gem(0:imx,0:jmx,0:kmx),eden_gem(0:imx,0:jmx,0:kmx),ijpar_gem(0:imx,0:jmx,0:kmx),ejpar_gem(0:imx,0:jmx,0:kmx),phi_gem(0:imx,0:jmx,0:kmx),Ah_gem(0:imx,0:jmx,0:kmx),As_gem(0:imx,0:jmx,0:kmx)
    write(stepid_str,'(I0.5)')n
#endif
    ...
    if(myid==0)then
      ...
      write(gemout,*)'start 3d out','n=',n,'cce_all_surface_node_number'
      call flush(gemout)

      cce_filename='out.3d.'//trim(stepid_str)//'.bp'
      if(init==0)then
        call adios2_declare_io(io,adios2obj,'output_3d',err)
        call adios2_define_variable(var, io, "iden_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "iden_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "ijpar_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "ijpar_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "eden_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "eden_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "ejpar_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "ejpar_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "phi_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "phi_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "Ah_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "Ah_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "As_gem", adios2_type_dp, 3, gdims_3d, goffset_3d, ldims_3d, adios2_constant_dims, err)
        call adios2_define_variable(var, io, "As_xgc", adios2_type_dp, 2, gdims_2d, goffset_2d, ldims_2d, adios2_constant_dims, err)
        init=1
      endif
      call adios2_open(engine,io, cce_filename, adios2_mode_write, mpi_comm_self, err)
      call adios2_begin_step(engine, adios2_step_mode_append, err)
   ...
   den_tmp=den(1,:,:,:)/q(1)*nu
   call mapping_GEM_XGC_new(iden_gem,iden_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then 
      call adios2_put(engine, "iden_gem", iden_gem, err)
      call adios2_put(engine, "iden_xgc", iden_xgc, err)
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   den_tmp=jpar(1,:,:,:)*e*nu*vu
   call mapping_GEM_XGC_new(ijpar_gem,ijpar_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then
      call adios2_put(engine, "ijpar_gem", ijpar_gem, err)
      call adios2_put(engine, "ijpar_xgc", ijpar_xgc, err)
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   den_tmp=dene*nu
   call mapping_GEM_XGC_new(eden_gem,eden_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then
      call adios2_put(engine, "eden_gem", eden_gem, err)
      call adios2_put(engine, "eden_xgc", eden_xgc, err)
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   den_tmp=upar*e*nu*vu
   call mapping_GEM_XGC_new(ejpar_gem,ejpar_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then
      call adios2_put(engine, "ejpar_gem", ejpar_gem, err)
      call adios2_put(engine, "ejpar_xgc", ejpar_xgc, err)
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   den_tmp=phi*Tu/e/real(ntube)
   call mapping_GEM_XGC_new(phi_gem,phi_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then
      call adios2_put(engine, "phi_gem", phi_gem, err)
      call adios2_put(engine, "phi_xgc", phi_xgc, err)
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   den_tmp=aparh_save*Tu/vu/e/real(ntube)
   call mapping_GEM_XGC_new(Ah_gem,Ah_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then
      call adios2_put(engine, "Ah_gem", Ah_gem, err)
      call adios2_put(engine, "Ah_xgc", Ah_xgc, err)
   endif
   call mpi_barrier(mpi_comm_world,ierr)
   den_tmp=apars*Tu/vu/e/real(ntube)
   call mapping_GEM_XGC_new(As_gem,As_xgc,den_tmp,grid,mapping_GEM_XGC_linear_coef)
   if(myid==0)then
      call adios2_put(engine, "As_gem", As_gem, err)
      call adios2_put(engine, "As_xgc", As_xgc, err)
   endif
   if(myid==0)then
      call adios2_end_step(engine, err)
      call adios2_close(engine, err)
      write(gemout,*)'end 3d out',' n=',n
      call flush(gemout)
   endif
#endif
